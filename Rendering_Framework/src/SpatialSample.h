...
public:
	...
	inline const float* position(const int idx) const { return this->m_buffer + idx * _DIM; }
...
public:
	static SpatialSample* importBinaryFile(std::istream& inputStream) {
		...
		float* buffer = new float[numSample * _DIM];

		inputStream.read((char*)(buffer), sizeof(float) * numSample * _DIM);
